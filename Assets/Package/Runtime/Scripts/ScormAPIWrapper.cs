...
namespace VARLab.SCORM
{
	...
	public class ScormAPIWrapper
	{
		...
		{
			Debug.Log("Initializing SCORM API Wrapper");
			IsScorm2004 = true;
			...
			IsScorm2004 = Convert.ToBoolean(result);
			if (IsScorm2004)
		...
		void processQueue()
		{
			try
			{
				lock (ResponseQueue)
				{
					...
					{                                           //Only process if we have something in the queue
		...
		{
			Debug.Log("ScormAPIWrapper: " + text);
		}
	}
}